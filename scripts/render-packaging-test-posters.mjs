...
import { packagingTestPosters } from "./packaging-test-poster-data.mjs";

const ROOT = process.cwd();
const VISUAL_DIR = path.join(ROOT, "output", "imagegen", "packaging-test", "visuals");
const POSTER_DIR = path.join(ROOT, "output", "imagegen", "packaging-test", "posters");
const WIDTH = 1080;
const HEIGHT = 1440;
...
async function renderPoster(poster, index) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">
    ...
    <text x="24" y="35" fill="${colors.white}" font-size="18" font-weight="700" font-family="Microsoft YaHei, sans-serif">半导体封装与测试精度 <tspan fill="${colors.violet}">|</tspan> Packaging &amp; Test</text>
    ...
    <text x="24" y="1330" fill="#d4c9e7" font-size="14" font-family="Microsoft YaHei, sans-serif">资料框架：DISCO · Besi · ASMPT · Onto · Nordson · FormFactor · Advantest</text>
    ...
    <text x="1056" y="1355" text-anchor="end" fill="#8e83a2" font-size="12" font-family="Arial">KAIROS SEMI · PACKAGING &amp; TEST SERIES</text>
    <rect x="0" y="1386" width="1080" height="54" fill="#05030b"/>
    <text x="24" y="1420" fill="#a89eb8" font-size="14" font-family="Microsoft YaHei, sans-serif">精密制造行业能力地图 · 半导体封装与测试</text>
...
const outputs = [];
for (let index = 0; index < packagingTestPosters.length; index += 1) {
  outputs.push(await renderPoster(packagingTestPosters[index], index));
}
